...
public abstract class MyBitmapLoader extends BitmapDecoder {
    ...
    protected MyBitmapLoader() {
        super();
        mOptions = OPTIONS.obtain();
        mOptions.inScaled = false;
    }

    protected MyBitmapLoader(MyBitmapLoader other) {
        super(other);
    ...
    private Bitmap decodeInMemory(TwiceReadableInputStream in) {
        Bitmap bitmap = MyBitmapLoader.from(in).scaleBy(mRatioWidth, mRatioHeight).decode();
        if (bitmap == null) return null;

        mAdjustedWidthRatio = mAdjustedHeightRatio = 1;

        BitmapDecoder decoder = MyBitmapLoader.from(bitmap);
        if (mRegion != null) {
    ...
    @SuppressLint("NewApi")
    @Override
    public MyBitmapLoader mutable(boolean mutable) {
    ...
    @Override
    public Bitmap createAndDraw(int width, int height, Rect rectDest,
                                Drawable background) {
    ...
    @Override
    public MyBitmapLoader useBuiltInDecoder(boolean force) {
        this.mUseBuiltInDecoder = force;
        return this;
    }

    @Override
    public MyBitmapLoader config(Config config) {
    ...
    public boolean equals(Object o) {
        if (!(o instanceof MyBitmapLoader)) return false;

        final MyBitmapLoader d = (MyBitmapLoader) o;
    ...
    public MyBitmapLoader id(Uri uri) {
        return id((Object) uri);
    }

    public MyBitmapLoader id(String uri) {
    ...
    @Override
    public MyBitmapLoader filterBitmap(boolean filter) {
        mScaleFilter = filter;
        mHashCode = 0;
        return this;
    }
    ...
    @SuppressWarnings("UnusedDeclaration")
    public MyBitmapLoader quality(Quality quality) {
